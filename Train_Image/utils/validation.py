...
def CCC_score(x, y):
    vx = x - np.mean(x)
    vy = y - np.mean(y)
    rho = np.sum(vx * vy) / (np.sqrt(np.sum(vx**2)) * np.sqrt(np.sum(vy**2)) +EPS)
    x_m = np.mean(x)
    y_m = np.mean(y)
    x_s = np.std(x)
    y_s = np.std(y)
    ccc = 2*rho*x_s*y_s/(x_s**2 + y_s**2 + (x_m - y_m)**2 +EPS)
    return ccc
def CCC_score_Torch(x, y):
    vx = x - torch.mean(x)
    vy = y - torch.mean(y)
...
class CCCLoss(nn.Module):
    ...
    def forward(self, x, y): 
        ...
        if self.digitize_num !=1:
            x = F.softmax(x, dim=-1)
            x = (self.bins.to(y.device) * x).sum(-1) # expectation
        x = x.view(-1)
