...
class ModelWrapper(object):
    def __init__(self, STModel, name, tasks, checkpoints_dir, 
        ...
        gpu_ids,
        AU_criterion, EXPR_criterion, VA_criterion, FA_criterion, VAD_criterion,
        lambda_AU, lambda_EXPR, lambda_VA, lambda_FA, lambda_VAD,
        is_train = True,
        cuda = True,):
        ...
        'EXPR': EXPR_criterion, 'VA': VA_criterion, 'FA': FA_criterion, 'VAD': VAD_criterion}
    ...
    def lambdas_per_task(self):
        ...
        'VA': self.lambda_VA, 'FA': self.lambda_FA, 'VAD': self.lambda_VAD}
        return lambda_dict
    ...
    def _init_prefetch_inputs(self):
        ...
        self._input_audio_length =  OrderedDict([(task, self._Tensor(self.batch_size, self.seq_len)) for task in input_tasks])
        self._input_audio = OrderedDict([(task, self._Tensor(self.batch_size, self.seq_len, int(self.window_size * self.sr))) for task in input_tasks])
    ...
    def _init_losses(self):
        ...
        criterions['VAD'] = Criterion().get(self._criterions_per_task['VAD'], 2)
    ...
    def set_input(self, input, input_tasks = None):
        ...
        for t in tasks:
            ...
            self._input_audio_length[t].resize_(input[t]['audio_length'].size()).copy_(input[t]['audio_length'])
        ...
        if (not self._is_train) or MC_dropout:
            ...
            for t in tasks:
                with torch.no_grad():
                    ...
                    input_audio_length = Variable(self._input_audio_length[t])
                    label = Variable(self._label[t])
                    output, hiddens = self._model(input_image, [input_audio, input_audio_length], hiddens)
                ...
                if VAD_teacher is not None and 'VAD' in self.tasks:
                    with torch.no_grad():
                        ...
                        loss_VAD = self._criterions_per_task['VAD'](F.log_softmax(output['VAD'].view(B*N, C), dim=-1), F.softmax(VAD_label.view(B*N, C), dim=-1))
