...
class Validator(object):
    def __init__(self):
        ...
        for i, dataset_name in enumerate(args.dataset_names):
            task = args.tasks[i]
            self.train_datasets[task] = DatasetFactory.get_by_name(dataset_name, 64, 30, window_size=args.window_size, sr = args.sr, train_mode = 'Train', 
            transform = test_transforms(args.image_size))
            self.val_datasets[task] = DatasetFactory.get_by_name(dataset_name, 64, 30, window_size=args.window_size, sr = args.sr, train_mode = 'Validation', 
            transform = test_transforms(args.image_size))
        ...
        predictions_file = 'N=5_student_round_1/new_annotation.pkl'
        ...
        matrix_file = "N=5_student_round_1/affinity_matrix.pkl"
    ...
    def eval_dataloader(self, dataloader, annotation_data, task):
        ...
        for i_batch, batch_data in tqdm(enumerate(dataloader), total = len(dataloader)):
            if (video_name is None) or (not video_name is None and batch_data['video'][0] != video_name):
                ...
                video_fps_dict[video_name] = int(batch_data['fps'][0])
                hiddens = dict([(i_model, dict([(t, None) for t in args.tasks])) for i_model in range(len(self._models))])
        ...
        for video_name in tqdm(unique_video_names, total = len(unique_video_names)):
            ...
            video_df = self.format_video_df(video_paths, video_frames_ids, video_labels, video_preds_dict, task, fps)
    ...
    def merge_two_dfs(self, df1, df2, on = 'frames_ids'):
        ...
        return merged
    def format_video_df(self, video_paths, video_frames_ids, video_labels, video_preds_dict, task, fps):
        ...
        annotation_dict.update({'fps': [fps] * total_length})
